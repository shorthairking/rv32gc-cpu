...
| MMU-12 | `csr/rv32_csr.v` | `L=1` 被锁定的项 | 写 `pmpcfg[i]`/`pmpaddr[i]` 被**忽略**（不报错）；`L=1 && A=TOR` 时 `pmpaddr[i-1]` 也被忽略；直到复位 | F |
| MMU-13 | `csr/rv32_pmp.v` + `csr/rv32_csr.v` | 粒度 G=0 | **G=0 ⇒ 4 字节粒度**：`pmpaddr` 的 32 位**全部可写、可原样回读**（低 2 位是 PA[3:2]，**不是恒 0** —— 本文旧表述已订正）；NA4 可选；NAPOT 区间 = `2^(k+3)` 字节（k = `pmpaddr` 低位连续 1 的个数；`pmpaddr` 全 1 ⇒ 覆盖全空间） | E |
| MMU-14 | `csr/rv32_pmp.v` | PMP 配置被写入 | **本核取指/访存检查是纯组合、不缓存 PMP 结果** ⇒ 无需任何失效动作（超规范，安全）；2A-4 引入 I-Cache/TLB 后若缓存"PMP 通过"标记，则必须在配置写入时失效（04 §5.3） | F（2A-4 起） |
| MMU-15 | `csr/rv32_pmp.v` | PTE 取指本身 | 也受 PMP 检查（物理地址访问）；本核 MMU 未实现（2A-4） | F（2A-4 起） |

**PMP 实现结构与语义（阶段 2A 已落地，取代本节旧口径）**：

* **分工**：状态与写路径在 `rtl/csr/rv32_csr.v`（`pmpcfg0-3`@0x3A0-0x3A3、`pmpaddr0-15`@0x3B0-0x3BF；
  `0x3A4-0x3AF` 不存在 ⇒ 非法指令）；匹配与权限判定在**纯组合**模块 `rtl/csr/rv32_pmp.v`。
* **判定规则**（`machine.adoc` Priority and Matching Logic / Locking and Privilege Mode）：
  ① **静态优先级 —— 编号最小的匹配项决定结论**（不是"所有匹配项都必须允许"）；
  ② 被选中项必须覆盖本次访问的**全部字节**，否则失败（与 L/R/W/X 无关）；
  ③ `L=0 且 M` ⇒ 通过；否则按访问类型查 R/W/X（AMO 需 R 与 W 同时满足）；
  ④ 无匹配项：M ⇒ 通过，S/U ⇒ 失败；
  ⑤ `A` 编码 `0=OFF / 1=TOR / 2=NA4 / 3=NAPOT`（**无保留编码**）。
* **与参考模型 Spike 对齐**（参考签名由它生成）：`pmpcfg` 字节写入只保留 `R|W|X|A|L`（保留位 [6:5]
  写忽略、读恒 0），且 **MML=0 时丢弃 `R=0,W=1` 的 W**；被锁定项的写被忽略但**不报错**；
  `pmpaddr` 全部 32 位可写（G=0）。
* **检查点**：**取指**在 ID 级按指令地址检查（32 位指令覆盖 pc 与 pc+2）⇒ cause 1，`tval`=该指令地址；
  **访存**在 MEM 的 `M_IDLE` **发起总线请求之前**检查 ⇒ 不通过则不拉请求、直接进 `M_DONE`，由 WB 级报
  cause 5/7（与 misaligned 同一写法）。⚠ **禁止用"闸掉请求"实现拒绝**：不发请求又不进 `M_DONE` 会让
  `mem_stall` 恒 1 ⇒ `advance_all` 恒 0 ⇒ 死锁且永不进 trap。
* **旁通**：`pmpcfg/pmpaddr` 的 WB→ID 读旁通必须返回**锁定/归一之后的有效值**（不能直接回 `csr_wdata`）。
* **性能约束**：匹配引擎是每拍被反复求值的组合逻辑 ⇒ NAPOT 掩码必须**无循环**实现，且访存侧只许有
  **一个**匹配引擎（一个实例带两组权限需求 `permit`/`permit2`）；复制匹配树会让 iverilog 仿真慢 3 倍以上。
