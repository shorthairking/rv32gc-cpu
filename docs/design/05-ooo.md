...
- 目标寄存器 `rd`：从空闲表分配新物理号 `pd`；ROB 表项记录 `rd_old`（提交时回收）。
...
4. 若更老 store 地址未就绪：
   - 预测"无冲突" → 允许 load 先行访存；随后违例检测发现重叠 → **replay**：清空该 load 及其所有更年轻指令，重新执行（代价约 10~15 周期）；
   - 预测"有冲突" → 等待，直到更老 store 地址就绪；
5. store：地址与数据在 LSQ 中就绪，**只有 ROB 提交时才写入 D-Cache**（保证精确异常与推测安全）。

**replay 的实现要点**：replay 采用"部分清空"（flush younger than load）而非全流水清空，roi 由该 load 的 `rob_idx` 决定；与分支误预测共用同一套清空/恢复硬件。
