...
| PRF 写端口 | 6 写端口（ALU0/ALU1/BRU 合并、MDU、LSU、FPU、CSR/系统），冲突时按优先级仲裁并插入 1 周期 |
...
| 路径 | 预算 | 措施 |
|---|---|---|
| IF0：PC → BTB 读 → 命中比较 → PC 选择 | 4.5 ns | BTB 用分布式 RAM，读出寄存器打拍；PC 选择器两级 |
...
若 4 发射配置在最坏路径上无法收敛，按以下顺序降级（每步都保持功能正确，只影响 IPC）：
1. 发射队列选择器由"单周期全并行"改为"两周期分级选择"；
2. L2 访问增加一级流水（容忍 +1 周期延迟）；
3. `CORE_WIDTH` 参数降为 2（双发射），仍满足"≥4 标量执行部件"与五级流水要求，主频目标不变。
