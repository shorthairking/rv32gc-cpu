...
- **下一步（停下等用户指令，按 §6 阶段纪律）**：2A 已收官。后续可选：**2B**（4 发射乱序后端、锦标赛预测器、11 级流水，与顺序核锁步比对）或**阶段三**（U-Boot 移植：board/defconfig/DTS + NAND 驱动 + saveenv + 从 NAND 自动启动）。上板口径维持 33 MHz 同域（用户已定：暂不提速）。
