...
- **下一步（M5 上板，第一轮已发现并修复 R2 缺陷）**：✅ **R2 修复已提交（`7a06825`）**——AXI R 握手拍锁存 rdata_hold，修上板 DDR3 uncached 读晚 2 拍采样（上板现象：字符串/FREQ/LED/开关正常、十进制常量全乱、RESULT BAD 循环重跑）；步④ 判据已对 Flash 延迟鲁棒化；新 bitstream `rv32gc_chiplab_soc.bit` md5 `f375baa04d8e495cfbb8c5df2042adf4`、新镜像 `m5_board.bin` md5 `b82be1fc4f67d029232a6409f6941b33`（1773 B）。**用户按 `sw/M5-board-runbook.md` 重烧：新 bitstream + 新 bin（Flash 地址 0），断电重上电，115200 8N1 观察五步到 `RESULT: RV32GC-M5-OK`；失败抄回手册 §5.1 十项信息。**
