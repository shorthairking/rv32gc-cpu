...
- **两线已恢复**（send_message）：`8009c161`（2B-2 第 18 轮：发射侧修法 B28 + 修法 (c) 重落地 + 三程序全绿 + regress 30/30）；`501a646d`（L-m3：核对编译状态 → nand_boot_test.py 零注入冷启 → 持久化终验 → README §9 全绿）。现场与下一步口径以暂停存档点 #2 为准（见下，仍适用）。
