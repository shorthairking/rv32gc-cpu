...
### 1.5.1 ✅ 恢复执行（2026-09-23 暂停存档点 #5 后，用户指示"请继续CPU的开发"）

- **4b-1 已恢复**（send_message `d9ce4e74`；现场核对：整设计可编译 0 error）。任务书/判据/口径以存档点 #5 为准（csr_file 替换、trp_csr_* 删净、适配层三要点、PTE-PMP 以 2A RTL 为准）。
- **4b-1 未提交现场（恢复时先核对工作树）**：rtl/back2/backend_top.v、sim/unit/prog/{back2_lockstep_data.svh,gen_back2_lockstep_data.py}、sim/unit/tb_core_top_2b.sv 有改动（4b-1 csr_file 替换的进行中状态；**恢复时先确认整设计可编译**，若停在半截先让代理修复到可编译再继续）。4b-1 任务书：csr_file+priv_ctrl+trap_ctrl+pmp_check 替换 b2_csr 过渡栈、删净 4a 自造 trp_csr_*、适配层三要点（ROB 头当 W 拍 / exc_valid 与 commit_valid 相与 / CSR 写数据 rdata_w 旁路）、mstatus FS/SD 合并层搬移；判据=全程序 68 项不回退 + CSR 读写轨迹对黄金 + regress 32/32 + 两套 CSR 不并存。口径裁决：PTE-PMP 以 2A RTL 为准（取指遍历 X、数据侧 LOAD/STORE）。
- **已收官 tags（最新链）**：…、2B-4（d7932a3）、2B-4.2（e32c608）、2B-4.3（3dd93fa）、2B-4.4（09b5e32+d8b3600）、2B-4.5（c7fb923，向量模式+D4 缺陷，tb_core_top_2b 68/68、regress 32/32 母代理亲跑）。B 线 tags：B5-QEMU/L2-NAND/L3-UBIFS/B-TFTP。
- **后续段**：4b-1（csr_file 替换）→ 4b-2（Sv32 全链路：satp/PTW 串行复用/A-D 写通路/I-D 翻译/sfence.vma+cbo）→ 4b-3（PLIC+S 模式 p10_priv）→ 4c（fcsr+D 扩展）→ 2B-5 时序收敛上板。
- **备份/日志**：../.b2chk/（*.s13-*.s15 等）、docs/chiplab-qemu/mac/logs/、docs/linux-port/logs/。恢复方式：同会话 send_message `d9ce4e74`；跨会话按 back2_report.md §B4.5 与上表 fresh dispatch。
