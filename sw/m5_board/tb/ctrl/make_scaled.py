...
#==============================================================================
# 目的：被冻结的 m5_board.S 在 RTL 仿真里**跑不完**（一轮主循环 = 8×`DELAY_HB_TICKS`
#       + `DELAY_SW_TICKS` + UART 输出 ≈ 7.4e7 拍 ⇒ iverilog ≈2.5e3 拍/秒下 ≈9 小时墙钟）。
#       本脚本对它做**纯机械、可核对**的 **2 处"挂钟等待常量"缩放**，其余字符
#       （地址口径、判据常量、字符串、五步 + 步2.5 结构、puts_*/udiv …）逐字不动，
#       从而在可承受的仿真时间里观察"程序逻辑本身会判 OK 还是 BAD"。
#
# 2 处替换（每处都断言**恰好命中 1 次**，否则非零退出 —— 拒绝静默改错）：
#   ① `.equ DELAY_HB_TICKS, 8000000`  → `100000`（步① 心跳间隔；只决定"停多久"）
#   ② `.equ DELAY_SW_TICKS, 10000000` → `200000`（步③ 停留；只决定"停多久"）
#   ★ **不缩放**任何参与判定的量（MEAS_ITER / XIP_CPI / CPI_SANITY_* / R2 探针地址与模式），
#     也不改 puts_char 的 LSR 等待上限（诊断版打印已改为纯寄存器路径，上限 64 在仿真里
#     每字符 ≈2.7e3 拍 —— 保持"只改挂钟等待"的最小偏离）。
#   ⇒ 缩放副本的步①..步⑤ + 步2.5 判据与冻结程序**逐字相同**（差异机器证据见 diff_equiv.py）。
#
