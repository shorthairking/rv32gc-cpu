...
module csr_file (
    ...
    //--------------------------------------------------------------------------
    // ★ T2（2026-09-18）新增：xRET 重定向专用的 *epc 出口（**组合，直取寄存器**）
    //--------------------------------------------------------------------------
    //   动机（时序，不改语义）：xRET 的 PC 重定向原先借用**唯一组合读口**
    //   （`raddr` 切到 mepc/sepc），于是 `xret_epc_pc = rdata` 成了
    //   「全部 CSR 寄存器 → 读口大 mux → pc_gen → pc_r/D」的超长组合链。
    //   这里把 mepc/sepc 单独引出一个出口，取值与读口口径**逐位相同**
    //   （`{*epc[31:1],1'b0}`，见本文件 §5 的 `RV32GC_CSR_MEPC/SEPC` 分支），
    //   但只经过一级 mux ⇒ 重定向路径长度大幅缩短。
    //   · 只增出口、不动任何既有端口/判据（sim/unit/tb_csr_file.sv 不受影响）。
    ...
    //   ★ 取值口径 = 读口（`rdata_r`）对同地址的分支：`{reg[31:1], 1'b0}`。
    //     两个出口都是**寄存器直出**（无地址译码、无旁路），供 xRET 单拍使用。
    //     （本 assign 必须位于 §3 的寄存器声明之后 —— 否则又制造一处隐式声明。）
    assign mepc_o = {mepc_r[31:1], 1'b0};
    assign sepc_o = {sepc_r[31:1], 1'b0};
